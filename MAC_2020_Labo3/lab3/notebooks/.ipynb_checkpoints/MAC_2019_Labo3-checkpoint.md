...
```scala
rddMovies.map(m => m.title).filter(m => m.contains("City")).foreach(println)
```
...
val rateMin = 0
val rateMax = 5
rddMovies.filter(m => m.rating <= rateMax && m.rating > rateMin).map(m => (m.rating, m.title)).sortBy(_._1, ascending = true).take(10).foreach(println)
```
...
```scala
rddMovies.flatMap(m => m.genres).map(m => (m.trim, 1)).reduceByKey( _ + _ ).sortBy(_._2, ascending = false).take(10).foreach(println)
```
